...
struct Modem::Data {
      ...
      while (do_read) {
        ...
        if (ret > 0) {
          if (fds[0].revents & POLLIN) {
            ...
            if (bytes_read < 0) {
              syslog(LOG_ERR, "MODEM: error reading from file descriptor: %s",
                     std::strerror(errno));
              break;
  ...
  void power_modem() {
    ...
    int r =
        ioctl(modem_pwr_ctrl.fd, GPIOHANDLE_GET_LINE_VALUES_IOCTL, &pwr_data);
    if (r < 0) {
    ...
    r = ioctl(modem_flight_ctrl.fd, GPIOHANDLE_GET_LINE_VALUES_IOCTL,
              &flightmode_data);
    if (r < 0) {
    ...
    r = ioctl(modem_flight_ctrl.fd, GPIOHANDLE_SET_LINE_VALUES_IOCTL,
              &flightmode_data);
    if (r < 0) {
  ...
  bool write_command(std::string cmd, std::string expected_response) {
    ...
    if (bytes_written < 0) {
      std::cerr << "failed to write: " << cmd << std::strerror(errno)
                << std::endl;
      return false;
    }
  ...
  void handle_message(const std::string &msg) {
    ...
    if (msg.find("+CRING: VOICE") != msg.npos || msg.find("RING") != msg.npos) {
      ...
      try {
        // Stops writing during the handler and allows the modem to be able to
        // flush its buffers
        call_in_progress = false;
        call_is_incoming = false;
        // call_ended_handler();
  ...
  ssize_t write_command(const std::string &command) {
    ...
    if (bytes_written <= 0) {
      syslog(LOG_ERR, "MODEM: failed to write command: %s",
             std::strerror(errno));
    }
  ...
  void init_serial(const std::string &serial_device, int baudrate) {
    ...
    if (serial_fd == -1) {
      throw std::runtime_error(std::string("failed to open serial device: ") +
                               std::strerror(errno));
    }
    ...
    if (ret < 0) {
      throw std::runtime_error(std::string("failed to set baudrate: ") +
                               std::strerror(errno));
    }

    ret = tcsetattr(serial_fd, TCSANOW, &config);
    if (ret < 0) {
      throw std::runtime_error(std::string("failed to set termios config: ") +
                               std::strerror(errno));
    }
  }
...
void Modem::AnswerCall() { m_data->write_command("ATA\r\n"); }
...
bool Modem::HasDialtone() { return true; }

bool Modem::CallIncoming() { return m_data->call_is_incoming; }

bool Modem::CallInProgress() { return m_data->call_in_progress; }

void Modem::SetModemEventHandler(ModemEventHandler handler) {
  m_data->m_event_handler = handler;
}
